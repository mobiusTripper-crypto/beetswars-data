...
`"rewardcap"` \* - Number. Overall reward is capped at the given value in USD or in native tokens, if not USD based. If applied to a combined reward, the rewardcap limits the sum of all rewards. Add fixed reward amout to a percent/pervote rewardcap to get the correct value.
...
V 0.51 on 2022/09/05 - clarify rewardcap
